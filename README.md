...
- 可选 1–9 级压缩强度（默认 6），并在处理完成前显示预估体积
...
> 强度模式会按所选级别控制 PNG 调色板颜色数量并采用更小结果；目标大小模式会比较多个调色板候选，采用不超过目标且调色板最丰富的版本，而不是一味选择最小文件。调色板量化会合并肉眼较难区分的近似颜色，因此属于有损压缩；动画 PNG 始终走无损路径。其他格式的效果取决于源文件中的可移除元数据。
